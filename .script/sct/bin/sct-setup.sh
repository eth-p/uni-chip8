...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# SCT: Setup Script
# This script will attempt to set up the packages needed for SCT to run.
# ----------------------------------------------------------------------------------------------------------------------
...
if ! [[ -f "${TEMP}/sct-setup" ]]; then
	...
	# NPM.
	colorify <<-END
		{style notice}Performing first-time set-up.{/style}
		{style notice}This might take a while...{/style}
	END

	npm install || {
		warn --fatal "NPM failed to install one or more packages."
		warn_details "{code}sct{/code} might not run correctly (or at all)."
		warn_done
	}
	...
	# Done.
	colorify "{style notice}Everything is done!{/style}\n"
